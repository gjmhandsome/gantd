...
if (!fs.existsSync(path.resolve(__dirname, '../../gantd'))) {
  console.error('未找到迁移目标项目：gantd，请检查后重试');
  return;
}
...
compPaths.forEach(pathItem => {
  const targetPath = pathItem.replace('/gantd-demo/src/components', '/gantd/src');
  try {
